...
|---|---|---|
| mat[0][1] | 4| 0 and 1 adjacent with weight of 4 |
| mat[1][0] | 4| 0 and 1 adjacent with weight of 4 |
| mat[0][3] | 0| 0 and 3 not adjacent |
| mat[3][5] | 7| 3 and 5 adjacent with weight of 7 |
...
100   4950    81600             40000         0.5x
500   600     21600             1000000        46.3x
500  124750   2008000           1000000         0.5x
...
> Your answer: In the first case, the Adjacency list is smaller, with the second case, an Adjacency list is more efficient, the cutoff between the two being 2500 edges, using E < V^2/4
...
> Your answer: Tuples are immutable, and so are hashable in that they can be used as keys in a map structure, while also being more memory efficient lacking the overhead normally used for expanding the size of a structure like a list adt. In this case there is no need to change the game states; in fact this would be a detriment, so making them tuples is both safer and more efficient.
...
> Your answer: Because this algorithm calculates costs by the number of steps, not the cost of each step. So traversing one egde with a weight of 100 would be chosen over 3 edges with weights of 1 each. As long as the actions are equal this is an efficient algorithm; it will determine the fewest steps to the goal.
...
> Your answer: Each node of each depth level is visited, and every adjacent node is added to the frontier queue as the search proceeds. This implies an exponential space complexity (checking I found B^d, branching to depth level). Larger problems will take more compute power and memory to complete in a reasonable amount of time.
...
> Your answer: The BFS frontier maxed at 25134 nodes, while DFS max size reached 85 nodes. DFS is only placing nodes adjacent to each visited node pulled from the stack into memory as visited nodes, so it is not mapping the entire graph. In contexts where memory is limited and resources applied to following the solution delivered by DFS are not, DFS would be the clear choice.
...
> Your answer: The solution found by BFS was 33 moves, while that of DFS was 77. DFS follows a path down the graph to one potential solution; whether this is the shortest or not is not a factor. A solution might be a few moves from the start, but if the DFS follows a parallel path to the solution, the returned solution could be much larger.
...
> Your answer: The limit prevents DFS from following very long paths to a solution when shorter ones are possible. It is also possible for DFS to wander forever down one branch without returning to explore any others, meaning it is not complete.
...
> Your answer: BFS uses a queue to organize a first in first out search of every node in the expanding frontier before moving a level deeper. UCS by contrast employs a priority queue to derive a min heap of edge values, which means the shortest set of moves will be at the beginning of the queue, and the node with the currently lowest cost will be expanded first.
...
> Your answer: UCS, using the min heap, selects the path with the lowest total edge cost, which may be longer than a direct route. In this case it avoided the higher cost tiles at the centre of the grid.
...
> Your answer: In instances where the value of every move is a constant, both algorithms will perform about the same, as the min heap of UCS will not provide a path any more efficient than that of BFS. Where the domain is over an interval of values, BFS will not take these into account, and UCS by contrast will find the lowest cost route, even if not the most direct. The eight puzzle game would be solved equally well by both, while finding the shortest route on an active traffic map would not be solved equally well by both.
...
> Your answer: At E=100V Bellman Ford is nearly 500 times slower that Dijkstra. The logarithmic factor in Dijkstra's compexity equation caps it's growth as E density increases.
...
> Your answer: As there are no negative edge weights, Dijkstra's algorithm would seem to be the best fit. Bellman-Ford and Floyd-Warshall would be excluded due to longer processing times; although E = 3V is still relatively sparse, it's really the absolute size of the inputs that matters most, and at this scale particularly Floyd-Warshall will become unusable for the task.
---
