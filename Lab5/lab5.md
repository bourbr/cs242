...
> Your answer: Five of eleven slots are used with six collisions, they seem clustered toward the middle of the table, with a couple outliers in slot 10.
...
> Your answer: The hash value would be the sum of those ascii characters, which would be the same total for any anagram of those characters; for example, Hash and Shah. 
...
> Your answer: Prime numbers are divisible by themselves and 1. If we made the table 10 spaces long, modulo 10 would often return 0 causing crashes at that location, and depending on the structure of the hash table, this could become a linked list at 0 or cause probing from that point with every new value addition.
...
> Your answer: Chaining can tolerate a higher load factor, but the hash table will become less efficient, losing O(1) time complexity for the O(n) of linked list linear search. Load factors of 2, or n/m, that is input divided by table size, are tolerable with chaining. For open-addressing, probing is used to find empty spaces, but these must have a load factor less than 1, or the size of the table, as no nested data structures exist in which to store elements of an amount greater than m.
...
> Your answer: Seeing as the chain is a simple linked list, in which search is a linear process, prepending allows for only a head node, where appending would require both a head and a tail for each chain of the whole table. It could be that at scale the savings of one node accrues such that it's worth simply prepending. As the element is stored at that space, even with other elements, the particular order in the chain is not likely of any real significance.
...
> Your answer: It should probe each space which is occupied by any number of elements, so the number of probing operations (an operation in this context being walking a list from start to finish) would be equal to the number of occupied spaces, for a probe factor equal to lambda. If a chaining probing operation constitutes interrogation of each element in a slot, then the number of probes would be equal the number of elements (key-value pairs) in the table. 
...
> Your answer: Open addressing is limited by the size of the table, as the number of elements can not exceed the number of available spaces, one per space.
...
> Your answer: At a lambda of 2, with uniform hashing each slot should have a chain of two elements.
...
|---|---|---|---|
| carol (before delete) | 1| 1| |
| carol (after delete) | 1| 1| |
| alice | 10| 1| |
| grace | 4,5,6,7,8| 5| |
| zara | 7,8,9| 2| |
...
> Your answer: Because anywhere the key hashes into the cluster, its home will be at the first empty slot at the end of the cluster.
...
> Your answer: If Carol was part of the cluster, a hash to an empty location would indicate that the key Carol is not present in the table.
...
> Your answer: These tombstone slots, if the hash lands on them or near them and has to process through them to reach the key, create additional probes despite not containing any information. Rehashing the table resets these slots, and all the indices of the table are reorganized, which is a fairly expensive process, and so is hopefully minimized by good hash functions.
...
> Your answer: Open addressing does not implement a sub data-structure in the form of linked lists at each slot or bucket; the table size is discretely limited by the actual length of the table. With chaining, each slot can contain a linked list, and so could conceivably contain many more keys than the table length would ascribe, but in practice is usually capped at a lambda of 2, which would be about 2 elements per slot, or linked lists of size 2 per slot with perfect uniform distribution.
...
------------------------------------------------------------
       ...
       0.248           1.000           1.000           1.000
       0.495           1.140           3.520           1.280
       0.693           1.271           5.957           1.814
       0.891           1.356           8.756           2.322
...
> Your answer: By adding a second hash function, keys that would normally collide with the result of the primary hash are assigned to unique slots via the secondary hash. This should in theory dramatically reduce instances of collisions.
...
> Your answer: It seems like the optimal upper bound for a table using linear probing is a lambda of 0.5.
...
> Your answer: Double hashing results are fairly close to the theoretical efficiency thresholds. Linear probing results are quite a bit less efficient in reality than the theoretical predictions suggest. This could be due to the fact that the efficiency for linear probing is entirely dependent on the single function being 'masterfully' written, resistant to assigning the same slots to multiple keys. With the double hash structure, there is a redundancy that makes a high, discrete function efficiency less critical, as there are two working in parallel. 
